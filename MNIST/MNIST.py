...
path = 'C:/Users/19433/Desktop/MNIST'
train_images, train_labels = load_mnist_train(path)  # (60000, 784)、(60000,)
test_images, test_labels = load_mnist_test(path)  # (10000, 784)、(10000,)

features = np.vstack((train_images, test_images))  # (70000, 784)
labels = np.hstack((train_labels, test_labels))  # (70000)
...
def QUBO_FS_classif_RF_1_vs_all(features, k, labels):
    results = {}
    selected_features_dict = QUBO_FS(features, k, labels)
...
results_random_fs = random_FS_classif_RF_1_vs_all(features, k=30, labels=labels)
results_qubo_fs = QUBO_FS_classif_RF_1_vs_all(features, k=30, labels=labels)
